...
class Attention2(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,stage = 3):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        if stage == 2:
            self.TDS = TDSLayer(4096,128)
        elif stage == 3:
            self.TDS = TDSLayer(1024,320)
        elif stage == 4:
            self.TDS = TDSLayer(256,512)
...
class TDSLayer(nn.Module):
    def __init__(self, dim1, dim2, deploy=False):
        super(TDSLayer, self).__init__()
        ...
        self.ln1 = nn.LayerNorm(dim2)
        self.branch1 = c_branch(dim1, deploy = deploy)

        self.branch2 = c_branch(dim2, deploy = deploy)

       
    def forward(self, x):
        #N*1*H && N*1*W
        x_h, x_w = self.pool_h(x).permute(0, 2, 1), self.pool_w(x)
        
        x_h = ((self.branch1(x_h).permute(0, 2, 1)))
        x_w = ((self.branch2(x_w).permute(0, 2, 1)))

        x_w = x_w.permute(0, 2, 1)

        attention = self.act(self.bn1(x_h*x_w))
...
class c_branch(nn.Module):
    ...
    def get_equivalent_kernel_bias(self):
        ...
        return kernal3 + kernal5 + kernalk#3->max + 5->max +k->max
...
class Block2(nn.Module):
    def __init__(self, stage, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1      = norm_layer(dim)
        ...
        self.attn       = Attention2(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio,stage=stage
        )
        self.norm2      = norm_layer(dim)
        self.mlp        = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[32, 64, 160, 256],
        ...
        self.block2 = nn.ModuleList(
            [
                Block2(
                    stage=2, dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.block3 = nn.ModuleList(
            [
                Block2(
                    stage=3, dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
